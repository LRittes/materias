...
vasilha = genVasilha(size, amountItems, amountAnts)
...
def initAnt(ant: Ant, k1, k2, alfa, cod):
    ...
    while epoch < maxEpoch or (ant.hasItem and epoch >= maxEpoch):
        ...
        #     print(f'ant {cod}: {epoch} - {end - start}')
...
if __name__ == '__main__':
    ...
    for idx, ant in enumerate(ants):
        thread = Thread(target=initAnt, args=(ant, k1, k2, alfa, idx))
    ...
    # vasilha = [[(data[0]['x'],data[0]['y'],data[0]['rot']),   (data[2]['x'],data[2]['y'],data[2]['rot']), ' ',' '],
    ...
    # k2 = 0.01
    # alfa = 2
    # item = (data[4]['x'],data[4]['y'],data[4]['rot'])
