...
def getData():
    data = []
    with open("./dataset_15rot.txt", "r") as file:
        res = file.readlines()
    for line in res:
        x, y, rot = line.split('\t')
...
def drawVasilha(vasilha, items, ants, remove=False):
...
def moveAnt(ant, vasilha):
    if random.randint(0, 1) == 0:
        x = random.randint(0, 1)
        if x == 0:
            newLin = ant.loc[0]
            newCol = moveV(ant, vasilha)
        else:
            newLin = moveH(ant, vasilha)
            newCol = ant.loc[1]
    else:
...
def initAnt(ant, vasilha, items, ants, lock, k1, k2, alfa, cod):
    epoch = 0
    start = time.time()
    while epoch < maxEpoch or (ant.hasItem and epoch >= maxEpoch):
        newLocAnt = moveAnt(ant, vasilha)
        ...
        drawVasilha(vasilha, items, ants)
        lock.release()
        
        if epoch % 1_000 == 0:
            end = time.time()
            
            print(f'ant {cod}, loc {ant.loc}, epoch {epoch}, time (seg) {end - start}')
...
if __name__ == '__main__':
    with Manager() as manager:
        ...
        for idx, ant in enumerate(ants):
            procs = Process(target=initAnt, args=(ant, vasilha, items, ants, lock, k1, k2, alfa, idx))
        ...
        with open(f'./{size}_{amountItems}_{amountAnts}_{k1}_{k2}_{alfa}_{maxEpoch}.txt', "a") as file:
            drawVasilha(vasilha, items, ants, True)
            print_vasilha(vasilha, file)
